...
### Test
def _test_leaderelection(N=4, f=1, seed=None):
    # Generate keys
    PK, SKs = dealer(N, f+1, random.seed())
    sid = 'sidABC'
    ...
    coins = [leader_election(sid, i, N, f, PK, SKs[i], sends[i], recvs[i]) for i in range(N)]
...
def test_when_signature_share_verify_fails():
    ...
    rnd = random.Random(seed)
    router_seed = rnd.random()
    sends, recvs = byzantine_router(N, seed=router_seed, node=2, sig_err=True)
