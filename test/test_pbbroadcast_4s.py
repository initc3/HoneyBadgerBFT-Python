...
from honeybadgerbft.core.pbbroadcast_4s import pbbroadcast_4s
...
def _test_pbbroadcast_s4(N=4, f=1, leader=None, seed=None):
    # Crash up to f nodes
    ...
    for i in range(N):
        ...
        t = Greenlet(pbbroadcast_4s, sid, i, N, f, leader, input, recvs[i], sends[i], broadcasts[i], PK, SKs[i])
        t.start()
        threads.append(t)
    ...
    for i,t in enumerate(threads):
        if i not in crashed and i != leader: assert t.value == m
...
def test_pbbroadcast_s4(N, f, seed):
    _test_pbbroadcast_s4(N=N, f=f, seed=seed)
#
#
# @mark.parametrize('seed', range(20))
